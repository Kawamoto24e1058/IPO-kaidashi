...
>
	...
	{#if data.dataSource === 'firestore'}
		<div class="sticky top-0 z-40 bg-amber-400/95 px-4 py-2 text-center text-sm font-medium text-amber-900 backdrop-blur-sm">
			⚠️ Notionに接続できないため、バックアップデータを表示中
			{#if data.backupDate}
				<span class="text-xs opacity-75">（{new Date(data.backupDate).toLocaleString('ja-JP')} 時点）</span>
			{/if}
		</div>
	{/if}
	...
	<div class="mx-auto max-w-md space-y-6 p-6">
		<div class="flex items-center justify-between mb-8 mt-4">
			...
			<div class="flex items-center gap-3">
				...
				<button
					onclick={async () => { await fetch('/api/logout', { method: 'POST' }); goto('/login'); }}
		...
		<div class="flex rounded-xl bg-gray-200/60 p-1 mb-6">
			<button
				...
				class="flex-1 rounded-lg py-2.5 text-[13px] font-bold transition-all {activeTab === 'shopping' ? 'bg-white text-[#0071E3] shadow-sm' : 'text-gray-500 hover:text-gray-700'}"
			>
				買い出し
			</button>
			<button
